...
// Type definition of heap allocated types.
...
struct Function {
  ...
  //
  // For embedding the user must ensure the name exists till the
  ...
  // Function can be either native C function pointers or compiled
  // functions.
  bool is_native;
  union {
...
struct Class {
  ...
  // For builtin type it'll be it's enum (ex: STRING, NUMBER, ...) for
  // every other classes it'll be vINSTANCE to indicate that it's not a
...
// define a function inside a header as long as it's static (but not a fan).
  ...
  static inline String* newString(VM* vm, const char* text) {
...
// Returns the VarType of the object type.
VarType getObjVarType(ObjectType type);

// Returns the ObjectType of the Var type.
ObjectType getVarObjType(VarType type);

// Returns the type name of the VarType enum value.
...
// Returns the VarType of the first class varaible [v].
