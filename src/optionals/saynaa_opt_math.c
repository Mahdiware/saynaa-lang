...
function(stdMathFloor,
  "math.floor(value:Numberber) -> Numberber",
  "Return the floor value.") {

  double num;
  if (!ValidateSlotNumber(vm, 1, &num)) return;
  setSlotNumber(vm, 0, floor(num));
}

function(stdMathCeil,
...
function(stdMathPow,
  ...
  double num, ex;
  if (!ValidateSlotNumber(vm, 1, &num)) return;
  if (!ValidateSlotNumber(vm, 2, &ex)) return;
  setSlotNumber(vm, 0, pow(num, ex));
}

function(stdMathSqrt,
...
function(stdMathRandom,
  "math.random(value:Number, value:Number) -> Number",
  "Return a random number.") {

  double num1, num2;
  if (!ValidateSlotNumber(vm, 1, &num1)) return;
  if (!ValidateSlotNumber(vm, 2, &num2)) return;
