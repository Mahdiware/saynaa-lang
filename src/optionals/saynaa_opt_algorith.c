...
bool checkSlotClosureOrBool(VM* vm, int argc, int slot, int* cmpSlot, bool* reverse) {
  if (argc >= slot) {
    ...
    } else if (GetSlotType(vm, slot) == vCLOSURE) {
      *cmpSlot = slot;
      return true;
...
static void _callfn(VM* vm, nativeFn fn) {
  int argc = GetArgc(vm);
...
void registerModuleAlgorithm(VM* vm) {
  Handle* algorithm = NewModule(vm, "algorithm");
  ...
  NewList(vm, 0);
  GetClass(vm, 0, 0);
  Handle* clsList = GetSlotHandle(vm, 0);
  ...
  releaseHandle(vm, clsList);

  registerModule(vm, algorithm);
  releaseHandle(vm, algorithm);
}