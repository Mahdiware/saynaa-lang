...
function(_ioWrite,
  ...
  double stream;
  if (!ValidateSlotNumber(vm, 1, &stream)) return;

  if (stream != 0 && stream != 1 && stream != 2) {
    SetRuntimeErrorFmt(vm, "Invalid stream (%g). Only use any of io.stdin, "
  ...
  const char* bytes;
  uint32_t length;
  if (!ValidateSlotString(vm, 2, &bytes, &length)) return;
...
function(_fileOpen,
  ...
  if (argc == 2) {
    if (!ValidateSlotString(vm, 2, &mode_str, NULL)) return;
...
function(_fileRead,
  ...
  if (argc == 1) {
    ...
    // but we need a long.
    double count_;
    if (!ValidateSlotNumber(vm, 1, &count_)) return;
...
function(_fileSeek,
  ...
  if (!ValidateSlotInteger(vm, 1, &offset)) return;

  if (argc == 2) {
    if (!ValidateSlotInteger(vm, 2, &whence)) return;
...
function(_open,
  ...
  int argc = GetArgc(vm);
  if (!CheckArgcRange(vm, argc, 1, 2)) return;
  if (argc == 1) setSlotString(vm, 2, "r");

  if (!ImportModule(vm, "io", 0)) return;           // slots[0] = io
  if (!GetAttribute(vm, 0, "File", 0)) return;      // slots[0] = File
  if (!NewInstance(vm, 0, 0, 0, 0)) return;         // slots[0] = File()
  if (!CallMethod(vm, 0, "open", 2, 1, -1)) return; // slots[0] = opened file
}
...
void registerModuleIO(VM* vm) {
  ...
  Handle* cls_file = NewClass(vm, "File", NULL, io,
