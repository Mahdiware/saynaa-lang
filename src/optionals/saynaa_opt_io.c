...
saynaa_function(
    _ioWrite, "io.write(stream:Var, bytes:String) -> Null",
    ...
    "io.stdout, io.stderr.") {
  double stream;
  if (!ValidateSlotNumber(vm, 1, &stream))
    return;

  if (stream != 0 && stream != 1 && stream != 2) {
    SetRuntimeErrorFmt(vm,
                       "Invalid stream (%g). Only use any of io.stdin, "
                       "io.stdout, io.stderr.",
                       stream);
    return;
  }
  ...
  const char* bytes;
  uint32_t length;
  if (!ValidateSlotString(vm, 2, &bytes, &length))
    return;
...
saynaa_function(
    _ioFlush, "io.flush() -> Null",
    "Warning: the function is subjected to be changed anytime soon.\n"
    "Flush stdout buffer.") {
  fflush(stdout);
}

saynaa_function(_ioGetc, "io.getc() -> String",
                "Read a single character from stdin and return it.") {
  char c = (char) fgetc(stdin);
  setSlotStringLength(vm, 0, &c, 1);
}
...
typedef struct {
  ...
  bool closed;         // True if the file isn't cl
} File;

void* _fileNew(VM* vm) {
  File* file = Realloc(vm, NULL, sizeof(File));
  ASSERT(file != NULL, "Realloc failed.");
  file->closed = true;
...
void _fileDelete(VM* vm, void* ptr) {
  File* file = (File*) ptr;
  if (!file->closed) {
...
saynaa_function(
    _fileOpen, "io.File.open(path:String, mode:String) -> Null",
  ...
  if (argc == 2) {
    if (!ValidateSlotString(vm, 2, &mode_str, NULL))
      return;

    // TODO: I should properly parser thiz.
    do {
      ...
      if (strcmp(mode_str, "rb") == 0) {
        mode = FMODE_READ_BIN;
        break;
      }
      if (strcmp(mode_str, "wb") == 0) {
        mode = FMODE_WRITE_BIN;
        break;
      }
      if (strcmp(mode_str, "ab") == 0) {
      ...
      SetRuntimeError(vm, "Invalid mode string.");
      return;
    } while (false);
  }
...
saynaa_function(
    _fileRead, "io.File.read(count:Number) -> String",
  ...
  if (argc == 1) {
    // Not using ValidateInteger since it's supports 32 bit int range.
    // but we need a long.
    double count_;
    if (!ValidateSlotNumber(vm, 1, &count_))
      return;
    if (floor(count_) != count_) {
      SetRuntimeError(vm, "Expected an integer.");
      return;
    }
    if (count_ < 0 && count_ != -1) {
      SetRuntimeError(vm, "Read bytes count should be either > 0 or == -1.");
      return;
    }
    count = (long) count_;
  }
  ...
  if (!(file->mode & FMODE_READ) && !(_FMODE_EXT & file->mode)) {
    SetRuntimeError(vm, "File is not readable.");
    return;
  }
  ...
  char* buff = Realloc(vm, NULL, (size_t) count + 1);
  ...
  if (read == 0) {
    setSlotStringLength(vm, 0, "", 0);
    goto L_done;
  }

  if (is_read_failed) {
    SetRuntimeError(vm, "C.fread() failed.");
    goto L_done;
  }

  // TODO: maybe I should check if the [read] length is larger for uint32_t.
  setSlotStringLength(vm, 0, buff, (uint32_t) read);
  goto L_done;
...
// something similler.
saynaa_function(
    _fileGetLine, "io.File.getline() -> String",
    "Reads a line from the file and return it as string. This function "
    "can only "
  ...
  if (file->mode & _FMODE_BIN) {
    SetRuntimeError(vm, "Cannot getline binary files.");
    return;
  }
  ...
  // A null byte '\0' will be added.
  setSlotStringLength(vm, 0, buff.data, buff.count);
...
saynaa_function(
    _fileWrite, "io.File.write(data:String) -> Null",
    "Write the [data] to the file. Since string support any valid"
    "byte value in it's string, binary data can also be written with "
    "strings.") {
  File* file = (File*) GetThis(vm);
  const char* text;
  uint32_t length;
  if (!ValidateSlotString(vm, 1, &text, &length))
    return;

  if (file->closed) {
    SetRuntimeError(vm, "Cannot write to a closed file.");
    return;
  }

  if (!(file->mode & FMODE_WRITE) && !(file->mode & FMODE_APPEND)
      && !(file->mode & _FMODE_EXT)) {
    SetRuntimeError(vm, "File is not writable.");
    return;
  }
...
saynaa_function(_fileClose, "io.File.close()", "Closes the opend file.") {
  File* file = (File*) GetThis(vm);

  if (file->closed) {
    ASSERT(file->fp == NULL, OOPS);
    SetRuntimeError(vm, "File already closed.");
    return;
  }
...
saynaa_function(
    _fileSeek, "io.File.seek(offset:Number, whence:Number) -> Null",
  ...
  if (!ValidateSlotInteger(vm, 1, &offset))
    return;

  if (argc == 2) {
    if (!ValidateSlotInteger(vm, 2, &whence))
      return;
    if (whence < 0 || whence > 2) {
      SetRuntimeErrorFmt(vm, "Invalid whence value (%i).", whence);
      return;
    }
  }
...
saynaa_function(_fileTell, "io.File.tell() -> Number",
  ...
  setSlotNumber(vm, 0, (double) ftell(file->fp));
}

// TODO: The docstring is copyied from io.File.open() this violates DRY.
saynaa_function(
    _open, "open(path:String, mode:String) -> Null",
    "Opens a file at the [path] with the [mode]. Path should be either "
    "absolute or relative to the current working directory. and [mode] can be"
    "'r', 'w', 'a' in combination with 'b' (binary) and/or '+' (extended).\n"
    "```\n"
  ...
  if (!CheckArgcRange(vm, argc, 1, 2))
    return;
  if (argc == 1)
    setSlotString(vm, 2, "r");

  if (!ImportModule(vm, "io", 0))
    return; // slots[0] = io
  if (!GetAttribute(vm, 0, "File", 0))
    return; // slots[0] = File
  if (!NewInstance(vm, 0, 0, 0, 0))
    return; // slots[0] = File()
  if (!CallMethod(vm, 0, "open", 2, 1, -1))
    return; // slots[0] = opened file
}
...
void registerModuleIO(VM* vm) {
  ...
  setSlotNumber(vm, 1, 0);          // slot[1]         =  0
  setAttribute(vm, 0, "stdin", 1);  // slot[0].stdin   =  slot[1]
  setSlotNumber(vm, 1, 1);          // slot[1]         =  1
  ...
  Handle* cls_file = NewClass(vm, "File", NULL, io, _fileNew, _fileDelete, "A simple file type.");
