...
static void _setSlotVector(VM* vm, int slot, int tmp, double x, double y) {
  ...
  if (!GetAttribute(vm, slot, "Vector", slot))
    return;
  if (!NewInstance(vm, slot, slot, 0, 0))
    return;

  setSlotNumber(vm, tmp, x);
  if (!setAttribute(vm, slot, "x", tmp))
    return;
  setSlotNumber(vm, tmp, y);
  if (!setAttribute(vm, slot, "y", tmp))
    return;
}
...
saynaa_function(_termSetPosition, "term.setposition(pos:types.Vector | {x, y}) -> Null",
