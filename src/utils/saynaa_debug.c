...
static void _printRed(VM* vm, const char* msg) {
  ...
  ByteBufferInit(&buff);
  {
    ...
    while (curr_line < end) {
      ...
      if (curr_line != line) {
        ...
        buff.count = 0;
        ByteBufferFill(&buff, vm, ' ', line_number_width);
        ByteBufferAddString(&buff, vm, " | ", 3);

        for (const char* c2 = line_start; c2 < at; c2++) {
          char white_space = (*c2 == '\t') ? '\t' : ' ';
          ByteBufferWrite(&buff, vm, white_space);
        }
...
static void _reportStackFrame(VM* vm, CallFrame* frame) {
  WriteFn writefn = vm->config.stderr_write;
